...
// Fetch data from the server using memberID
    fetch(`Fetch_MemberInfo.php?memberID=${memberID}`)
        ...
        .then(receivedData => {
            ...
            if (receivedData && typeof receivedData === 'object' && 'FeeInterval' in receivedData) {
                data = receivedData;
                originalPayType = data.FeeInterval; // Store the original value
    ...
    setValueById('M_Email', data.Email);
    ...
    const membershipDate = new Date(data.MemberShipDate);
    const currentDate = new Date();
    const daysDifference = dateDifferenceInDays(currentDate, membershipDate);

    // Disable mTypeSelect if the difference is less or equal than 346 days
    if (daysDifference <= 346) {
        mTypeSelect.disabled = true;
        console.log('MembershipType change is restricted.');
    } else {
        mTypeSelect.disabled = false;
    }
...
if (valid) {
    ...
    Econtact_name,Econtact_no, select, relOther);
            ...
            const feeIntervalFromDatabase = data && data.FeeInterval;

            console.log('Selected Pay Type:', selectedPayType);
            console.log('Fee Interval from Database:', feeIntervalFromDatabase);

            if (feeIntervalFromDatabase !== undefined && selectedPayType !== feeIntervalFromDatabase) {
                const confirmMessage = `Changing the payment interval to ${selectedPayType} will incur additional fees. Are you sure you want to proceed?`;

                const confirmation = window.confirm(confirmMessage);

                if (!confirmation) {
                    console.log("User canceled the payment interval change");
                    payTypeSelect.value = originalPayType; // Revert to the original value
